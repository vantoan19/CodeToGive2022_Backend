...
def upgrade() -> None:
    ...
    op.create_table(
        "job2label",
        sa.Column(
            "job_id", sa.Integer(), sa.ForeignKey("jobs.id"), index=True, nullable=False
        ),
        sa.Column(
            "label_id",
            sa.Integer(),
...
def downgrade() -> None:
    op.drop_constraint("job2label_label_fk", table_name="labels")
    op.drop_constraint("job2label_job_fk", table_name="jobs")
    op.drop_constraint("address_job_fk", table_name="jobs")
    op.drop_column("addresses", "job_id")
    op.drop_table("job2label")
    op.drop_table("jobs")
