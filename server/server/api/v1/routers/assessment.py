...
@router.post("/generate", response_model=schemas.Assessment, response_model_exclude={'tests':{'__all__': {'questions'}}})
...
def submit_assessment(
    *,
    db: Session = Depends(get_db),
    submit_info: schemas.AssessmentSubmit
) -> Any:
    ...
    assessment = assessment_crud.get_by_uuid(db=db, uuid=submit_info.uuid)
    return assessment_crud.update(db=db, db_obj=assessment, obj_in=assessment_update_info)

# @router.put("/", response_model=schemas.Assessment)
